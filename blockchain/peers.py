...
Ping => The initial message that a node sends to a
peer when initiating a connection

Transaction => A single transaction that propagates
throughout from peer to peer

Peers => A bunch of addresses that a peer may or
may not know about (to build their network)

Block => A single block (perhaps recently mined) for a
peer to add to their blockchain
...
class P2PProtocol:
    ...
    async def send_message(writer, message):
        # Send message to a particular peer (writer obj)
        writer.write(message.encode() + b"\n")

    async def handle_message(self, message, writer):
        # Handles message passed by the server 
